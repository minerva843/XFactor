...
class Ion_auth
{
	...
	public function register($identity, $password, $email, $additional_data = [], $group_ids = [],$project_id=false,$guest=false)
	{
		...
		$data_user = $this->ion_auth_model->register($identity, $password, $email, $additional_data,$project_id, $group_ids);
		
		$id = $data_user['id'];
		$new = $data_user['new'];
		//return $data_user;
		if (!$email_activation && !$project_id) 
		{
		...
		{		
			...
			if(!empty($additional_data['create_user_byadmin'])){
				return $data_user;
			}
			if(!$this->config->item('use_ci_email', 'ion_auth'))
			{  
			...
			//echo "11";  exit;
				...
				//echo "33";  exit;
				//$this->ion_auth_model->CreateUserSuccessMail($id,false,TRUE,$project_id);
				return $id;
				}
	...
	public function mattermostSignup($user_id,$username,$guest_id,$project_id)
	{
		$url = MMURL."users";
			$username1 = str_replace(" ", "_", $username);
			$post_data = array (
				"email" => $username1.$guest_id.$project_id.'@xfactor.com',
				"username" => $username1.$guest_id.$project_id,
				"nickname" => $guest_id,
