...
class Ion_auth
{
	 ...
	 * @author Mathew
	 */
	public function register($identity, $password, $email, $additional_data = [], $group_ids = [],$project_id=false,$guest=false)
	{
		...
		$id = $this->ion_auth_model->register($identity, $password, $email, $additional_data,$project_id, $group_ids);
		...
		{		
			...
			if(!$guest){
				...
				//print_r($email);  exit;
				//$this->forgotten_password($email);
				//return $id;
				
				$this->ion_auth_model->CreateUserSuccessMail($id,false,TRUE);
				return $id;
	...
	public function mattermostSignup($email,$username,$guest_id,$project_id)
	{
		$url = "http://54.179.99.134/api/v4/users";
			...
			$this->matterLogin($matterUser->username);
			//print_r($matterUser->id);print_r($this->session->userdata('token'));
			$curl = curl_init();

			curl_setopt_array($curl, array(
			  CURLOPT_URL => "http://54.179.99.134/api/v4/users/".$matterUser->id."/roles",
			...
			$post_dataa = array (
				"team_id" => '9xenpqfittf87bd5o7fehazbmw',
				"user_id" => $matterUser->id
			);
			$curll = curl_init();

			curl_setopt_array($curll, array(
			  CURLOPT_URL => "http://54.179.99.134/api/v4/teams/9xenpqfittf87bd5o7fehazbmw/members",
			  CURLOPT_RETURNTRANSFER => true,
			  CURLOPT_ENCODING => "",
			  CURLOPT_MAXREDIRS => 10,
			  CURLOPT_TIMEOUT => 0,
			...
			return $matterUser->id;
		
	}
	
	public function matterLogin($username)
	{
		$url = "http://54.179.99.134/api/v4/users/login";
	...
	function resendOtp($user){
		...
		$userd    = $this->ion_auth_model->user($user)->row();
		$data = [
				'identity'   => $userd->email,
				'id'         => $userd->id,
				'email'      => $userd->email,
                'otp'        => $otp
			];
		...
		$otp_response = $this->ion_auth_model->updateOtp($user,$otp,$additional);
		if($otp_response){
	...
	public function deactivate($id = NULL)
	{
        ...
        function retriveUserId($first_name,$last_name,$country_code,$mobile){
           // print_r($first_name); exit;
             return $this->ion_auth_model->retriveUserId($first_name,$last_name,$country_code,$mobile);
            
        }
        
        
        function project_guest_register_type($project_id){
