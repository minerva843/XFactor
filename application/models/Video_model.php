...
class Video_model extends CI_Model {
    ...
    public function insert($userdata){
		if($this->db->insert('xf_mst_video_stream',$userdata)){
	...
	function InsertFloorId($lastId,$array1){
		...
		if($this->db->update('xf_mst_video_stream', $array1))
		{
			return true;
		}
		else
	...
	function UpdateDropPoint($id,$floor_plan_drop_point){
		$this->db->where('id',$id);
		if($this->db->update('xf_mst_video_stream', array('floor_plan_drop_point'=>$floor_plan_drop_point)))
		{
			return true;
		}
		else
	...
	function UpdateData($id,$array){
		$this->db->where('id',$id);
		if($this->db->update('xf_mst_video_stream', $array))
		{
			return $id;
		}
		else
	...
	function FetchDataById($id){
		...
		$this->db->from('xf_mst_video_stream as p');
	...
	function DeleteDataByMultipleId($id){
		...
		$data=$this->db->delete('xf_mst_video_stream');
		if($data){
			return true;
		}else{
			return false;
	...
	function deleteAll($id=NULL)
	{
		...
		$query=$this->db->delete('xf_mst_video_stream');
		if($query){
			return true; 
		}else{
			return false;
	...
	function searchProgram($searchData=NULL,$searchDataOrderBy=NULL,$searchDataOrderByAsc=NULL,$AllSearchData=NULL,$project=NULL){
		...
		if(!empty($AllSearchData)){
			//for($i=0;$i<=count($searchAll);$i++){
				$this->db->group_start();
				$this->db->like('p.video_stream_name',$AllSearchData);
				$this->db->or_like('p.video_stream_source',$AllSearchData); 
				$this->db->or_like('p.video_stream_url',$AllSearchData); 
				$this->db->or_like('p.video_stream_id',$AllSearchData); 
