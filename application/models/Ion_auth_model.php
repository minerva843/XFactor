...
class Ion_auth_model extends CI_Model
{
	...
	function updateOtp($user_id,$otp,$additional){
		...
		$data = [
			'otp' => $otp,
			'otp_create_time'=> $additional['otp_create_time'],
			'otp_expire_time'=> $additional['otp_expire_time']
	...
	function getcountryinfo($id=NULL){
	...
	function FetchDataByWhere($where){
       ...
       function FetchDataByWhereWithOrs($where){
	 ...
	 * @author Mathew
	 */
	public function identity_check($identity = '',$project_id='')
	{
		...
		return $this->db->where($this->identity_column, $identity)
						...
						//->or_where('username',$user_name)
						->limit(1)
						->count_all_results($this->tables['users']) > 0;
		}else if(!empty($project_id)){
		
		$userdanamedata = $this->getUserByEmailPhoneUsername($identity,$project_id);
		if (!empty($userdanamedata)) {  
                 return TRUE;
	...
	// public function identity_check2($identity = '')
	// {
		// $this->trigger_events('identity_check');
		...
		// return $this->db->where($this->identity_column, $identity)
						// ->limit(1)
						// ->count_all_results($this->tables['xf_mst_users']) > 0;
	// }
	 ...
	 * @author    Mathew
	 */
	public function register($identity, $password, $email, $additional_data = [],$project_id, $groups = [])
	{
		...
		if ($this->identity_check($identity,$project_id) && empty($project_id)) 
		{ 
		...
		$query = $this->db->get_where($this->tables['groups'], ['group_name' => $this->config->item('default_group', 'ion_auth')], 1)->row();
		...
		$data = [
			...
			'otp_create_time'=> strtotime("now"),
			'otp_expire_time'=> strtotime("+5 min")
		];
		...
		$users_exist = $this->getUserByEmailPhoneUsername($identity);
		
		if(empty($users_exist)){
		
		$this->db->insert($this->tables['users'], $user_data);
		$id=$this->db->insert_id($this->tables['users'] . '_id_seq');
		// $mydata=$this->getUserByEmailPhoneUsername($identity);
		// $id=$mydata[0]->id;
		$data_return['new'] = 1; 
		$check_inser_group=1;
		...
		$userdanamedata = $this->getUserByEmailPhoneUsernameInGuest($identity,$project_id);
		if (empty($userdanamedata)) {  
                  $id = $users_exist[0]->id;
                  $data_otp = array(
                  'otp'=>$additional_data['otp'],
                  'password'=>$additional_data['password'],
                  'otp_create_time'=> strtotime("now"),
                  'otp_expire_time'=> strtotime("+5 min")
                  );
                  $this->db->update($this->tables['users'], $data_otp, ['id' => $id]);
	           $data_return['new'] = 0; 
		...
		if(!empty($project_id)){
		$userData=array('user_id'=>$id,'project_id'=>$project_id);
		
		}
		
		// add in groups array if it doesn't exists and stop adding into default group if default group ids are set
		if($check_inser_group==1){
		...
		return (isset($data_return)) ? $data_return : FALSE;
		//return $id;
	} 
	
	function JoinprojectId($id,$project_id=false){
	...
	/*
	public function register2($identity, $password, $email, $additional_data = [], $groups = [])
	{
		$this->trigger_events('pre_register');

		$manual_activation = $this->config->item('manual_activation', 'ion_auth');

		if ($this->identity_check2($identity))
		{
			$this->set_error('account_creation_duplicate_identity');
			return FALSE;
		}
		...
		// filter out any data passed that doesnt have a matching column in the users table
		// and merge the set user data and the additional data
		$user_data = array_merge($this->_filter_data($this->tables['xf_mst_users'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['xf_mst_users'], $user_data);

		$id = $this->db->insert_id($this->tables['xf_mst_users'] . '_id_seq');
...
*/
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login,first_name,xmanage_id,last_name,company,phone,username,gender,avatar,otp,display_name,designation')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
	...
	function CreateUserSuccessMail($id,$qrcode=false,$guest=NULL,$project_id=NULL,$new=1)
	{
		
		
		$this->db->select('users.*,xf_mst_country.name as country_name,xf_mst_country.phonecode');
		$this->db->from('users');
		$this->db->join('xf_mst_country', 'xf_mst_country.id = users.country_code','left');
		...
		$project=$this->JoinprojectId($id,$project_id);
		if(!empty($project)){ 
		
		$query = $this->db->get_where('xf_mst_project', array('id' => $project_id));
		$proj=$query->row();
		//$proj->project_name;
		...
		if($new){
		$sub='XPLATFORM –' .strtoupper($proj->project_name) .'  HAS INCLUDED YOU TO GUEST LIST';
		...
		$sub='XPLATFORM - REGISTRATION FOR '.strtoupper($proj->project_name) .' SUCCESSFUL.';
		
		
		}else{
		
		//$sub=strtoupper($proj->project_name) .' – REGISTRATION SUCCESSFUL. LOG IN DETAILS ARE LISTED BELOW';
		...
		$sub='XPLATFORM - REGISTRATION FOR '.strtoupper($proj->project_name) .' SUCCESSFUL.';
			...
			$sub='XPLATFORM – REGISTRATION SUCCESSFUL.';
			$project_name='';
		}
		
	
		$website=base_url();
		$forgotten_password_code = $this->forgotten_password($email);
		$data=array('project_name'=>$project_name,'project_id'=>$project_id,'username'=>$user->username,'email'=>$email,'password'=>$user->password,'website'=>$website,'project'=>$project,'salutation'=>$user->salutation,'gender'=>$user->gender,'first_name'=>$user->first_name,'last_name'=>$user->last_name,'alldata'=>$user,'qrcode'=>$qrcode,'guest'=> $guest,'project_type'=>$project_type,'phonecode'=>$phonecode,'forgotten_password_code'=>$forgotten_password_code);
		...
		if(strpos($project_type,"NO REG REQUIRED")){
			...
			if($new){
			$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_success_guest_no_reg_new', 'ion_auth'), $data, true);
			
			}else{
			
			$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_success_guest_no_reg_exist', 'ion_auth'), $data, true);
			...
			if($new){
			$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_success_guest_reg_new', 'ion_auth'), $data, true);
			}else{
			$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_success_guest_reg_exist', 'ion_auth'), $data, true);
			}
		
		}
		
		
		//$data['forgotten_password_code'] = $this->forgotten_password($email);
		
		//$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_success_guest', 'ion_auth'), $data, true);

		}else{
		
		$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_success', 'ion_auth'), $data, true);
		}
		...
		//$this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('account_creation_successful'));
	...
	function sendEmailerEmail($guest=0,$emailerid=0)
	{
		...
		$query1 = $this->db->get_where('xf_mst_emailer', array('id' => $emailerid));
		...
		$sub=$emailerdata->emailer_title;
		
		$data=array('emailerdata'=>$emailerdata,'guestdata'=>$guestdata);
		
		 
		$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('emailer_email_success', 'ion_auth'), $data, true);
		...
		$this->email->subject($sub);
		$this->email->message($message);
		if(!empty($emailerdata->pdf1)){
			$pdf1=base_url().'assets/emailer/'.$emailerdata->pdf1;
			$this->email->attach($pdf1);
		}
		if(!empty($emailerdata->pdf2)){
			$pdf2=base_url().'assets/emailer/'.$emailerdata->pdf2;
			$this->email->attach($pdf2);
		}
		if(!empty($emailerdata->pdf3)){
			$pdf3=base_url().'assets/emailer/'.$emailerdata->pdf3;
			$this->email->attach($pdf3);
		}
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.group_name, '.$this->tables['groups'].'.description')
	...
	public function set_session($user)
	{
		...
		$session_data = [
		    ...
		    'xmanage_id'                  => $user->xmanage_id, //everyone likes to overwrite id so we'll use user_id
	...
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = [])
	{
		...
		$existing_group = $this->db->get_where($this->tables['groups'], ['group_name' => $group_name])->num_rows();
		if($existing_group !== 0)
		{
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = [])
	{
		...
		if (!empty($group_name))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->db->get_where($this->tables['groups'], ['group_name' => $group_name])->row();
	...
	protected function _password_verify_sha1_legacy($identity, $password, $hashed_password_db)
	{
       ...
       function retriveUserId($first_name,$last_name,$country_code,$mobile){       
           $sql = "SELECT * from users where first_name='".$first_name."' AND last_name='".$last_name."' AND country_code='".$country_code."' AND phone='".$mobile."'";
        ...
        function getUserByEmailPhoneUsername($email){
        ...
        function getUserByEmailPhoneUsernameInGuest($email,$project_id){
        
        $sql = "SELECT email,id from xf_guest_users where project_id='".$project_id."' AND email='".$email."'  ";
           $query = $this->db->query($sql);
           $result = $query->result();
