...
//								$getUserAvatar = GuestDetails($user->id);
							...
							//	print_r($user);print_r('----');
								...
								//		print_r($user->mm_id);die;
										...
										$data .= '<div class="nearme-box clearfix Post_frontr_reapetguest pastsearchlist" data-val="' . $username . '" data-guest-type="' . $user->mm_id . '">';
										if ($user->avatar) {
				...
				if(!empty($_POST["workshop"])){
						...
						$.ajax({
						  ...
						  $.fancybox.open({
							...
							src         : "' . base_url() . 'chat/testchat",
							ajax: { 
							   settings: { data : "username="+username+"&workshop='.$_POST["workshop"].'", type : "POST" } 
							},
							touch: false,
						...
						$data .= '<script>$(".openSellerChatBox").click(function() {
							...
							$.fancybox.open({
								...
								ajax: {
									settings: {
										data: "username=" + grpID +"&workshop='.$_POST["workshop"].'",
										type: "POST"
									}
								},
								touch: false,
