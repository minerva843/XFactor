...
class Guest extends CI_Controller {
	...
	public function searchSingleDataWorkshop()
		{
			...
			if(empty($data1)){
						...
						$checkNull .='<p><span>CURRENTLY SELECTED: </span> NO GUEST SELECTED</p><div class="display-step-status" id="displaystatus" >
								 <h5>STEP 2 OF 3</h5>
			...
			if($data > 0)
			{
				...
				if(!empty($data->last_edited_username)){ $last_edited_username= $data->last_edited_username; }else{ $last_edited_username= "NIL";  }
					...
					$output .= '
						...
						<tr>
						<td colspan="2" class="top-fp"><p><span>CURRENTLY SELECTED :</span><span id="multipleGUESTSelect" style="color:black;" > '.$data->first_name.'</span></p></td>
					   </tr>
						
						
						<tr id="currentlyViewing" style="">
						<td  colspan="2" class="currentlyviewingclass"><p><span>CURRENTLY VIEWING :</span><span class="pname"> '.$data->first_name.'</span></p></td>
					    </tr>
		...
		public function searchSingleData()
		{
			...
			$checkNull='';
					if(empty($data1)){
						...
						$checkNull .='<p><span>CURRENTLY SELECTED : </span> NO GUEST SELECTED</p>';
					}
			$output = '';
			if($data > 0)
			{
	...
	function addstep1Post($id=false)
	{
		...
		$userdata = array();
	       $where = array(
			...
			//print_r($where);  exit;
		if(empty($id)){
		$userdata = $this->Guest_model->FetchDataByWhere($where);
		...
		$return['erroremail'] = "Email Already Exist";
	       ...
	       $where_part = array('phone' => $this->input->post('mobile'),'project_id'=>$_POST['project']);   
	       if(empty($id)){    
               $userdatamobile = $this->Guest_model->FetchDataByWhere($where_part);
               if(empty($id) && !empty($userdatamobile)){
		$return['erroremail'] = "Mobile Already Exist";
		
		}
		}  
		
		
		$where_part3 = array('username' => $this->input->post('username'),'project_id'=>$_POST['project']);   
	       if(empty($id)){    
               $userdatausername = $this->Guest_model->FetchDataByWhere($where_part3);
               if(empty($id) && !empty($userdatausername)){
		$return['erroremail'] = "Username Already Exist";
	...
	$type1=array("ONLINE REG REQUIRED","ONSITE REG REQUIRED","HYBRID REG REQUIRED","ONLINE NO REG REQUIRED","ONSITE NO REG REQUIRED","HYBRID NO REG REQUIRED");
		...
		$guest_registration_type =  'ONLINEAD';
				...
				$array['xguest_id'] = 'XPGS'.rand(1000000,99999999);
				...
				$result_email_exist = $this->Ion_auth_model->getUserByEmailPhoneUsername(trim($array['email']),trim($array['phone']),trim($array['username']));
...
$this->project_m->updatemm($mattrmostUser,$user_id);
	...
	function addstep3Post($id=false){ 
				...
				if($_POST['no_reg']==1){
			        ...
			        if($_POST['action']!='EDIT'){
			         ...
			         $this->Ion_auth_model->CreateUserSuccessMail($result_email_exist[0]->id,$qrcode,TRUE,$data_guest->project_id,$data_guest->new_u);
			        }
			        ...
			        $this->Ion_auth_model->updateUserTable($result_email_exist[0]->id,$data_us);
	...
	function GenarateQRCode($user_id=NULL)
	{
		...
		if(!empty($user_id) && !empty($project->project_id)){
			$url=base_url().'auth/login/'.$user->id.'/'.$project->project_id;
			...
			$url=base_url().'auth/login/'.$user->id;
			$filename = $TemUrl.'test'.md5($url).'.png';
			QRcode::png($url, $filename, $errorCorrectionLevel, $matrixPointSize, 2);
