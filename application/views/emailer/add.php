...
<form action="<?php echo base_url(); ?>program/addprogram" method="POST" id="addZone">
        <div class="top-header">
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="top-close"><input type="button" value="CLOSE" class="close-btn" name="submit" id="close-btnEmailStep1"></div>
                </div>
            </div>   
        </div>	   
				...
				<!-- 	<h5> ENTER ZONE DETAILS THAT ARE LISTED BELOW :</h5> -->
                    ...
                    <div class="zone-info add-conents">
						...
						<div class="form-group row">
							...
							<div class="col-sm-5">
								<input type="text" name="emailer_title" class="emailer_title" value="<?php if(!empty($data1->emailer_title)){ echo $data1->emailer_title; } ?>" class="form-control" id="emailer_title" placeholder="Enter EMAILER TITLE">
							</div>
						</div>
                       <div class="form-group row textarea-space poject_editor">
							...
							<textarea rows="4" cols="50" id="emailer_details" class="emailer_details" name="emailer_details" placeholder="ENTER EMAILER DETAILS"><?php if(!empty($data1->emailer_details)){ echo $data1->emailer_details; } ?></textarea>
							<script>
...
//for Emailer file size is restricted to 5 MB though standard is 15 MB	
	if(filextsion == 'pdf' || filextsion=='PDF'){
   ...
   $.ajax({
    ...
    beforeSend: function(){
     ...
     success: function (response) {
        ...
        autoSize  : true,
        type        : "ajax",
        src         : '<?=base_url();?>emailer/addGuestlist/'+data,
        ajax: { 
           settings: { data : 'project=<?php echo $project; ?>&group_id=<?php echo $group_id; ?>', type : 'POST' }
        },
        touch: false,
		 clickSlide: false,
        clickOutside: false
...
<script>
$('body').on('click', '#close-btnEmailStep1', function () {
            $.fancybox.close();
            
            <?php if(!empty($data1->id)){ ?>
           window.location.href = "<?php echo base_url(); ?>emailer/deleteJunkRecord/<?php echo $data1->id; ?>/<?php echo $group_id; ?>/<?php echo $project; ?>/ADD/1";
