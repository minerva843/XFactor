                    ...
                    <div class="tab right-tabs">
                        ...
                        <div class="table-content">	  
							...
							<div class="current-status">
								<p><span class="current-bold">CURRENTLY Selected : </span><?php if($data->audio_stream_name){ echo $data->audio_stream_name;}?></p>
								</div>
...
var audio_stream_url=$('#audio_stream_url').val();
 ...
 formdata.append('audio_stream_source', audio_stream_source);
   ...
   $.ajax({
     ...
     url: '<?php echo base_url(); ?>audio/addstepPOSt/<?=$data->id;?>', 
     type: 'post',
     data: formdata,
     dataType: 'json',
     contentType: false,
     ...
     success: function (response) {
			...
			if (data) {
				$.fancybox.open({
					src: '<?php echo base_url(); ?>audio/EditSuccessful/'+data,
					type: 'ajax',    
					opts: {
...
$("body").on('click','#btn5',function(){
   ...
   $.fancybox.getInstance('close');
	$.fancybox.open({
        ...
        ajax: { 
           settings: { data : 'project=<?=$data->project_id?>', type : 'POST' }
        },
        touch: false,
        clickSlide: false,
        clickOutside: false
...
//   //     $(this).after('<label id="videfiles-error" class="error" for="videfiles">ONLY 5 AUDIOS ARE ALLOWED.</label>')
//        alert("You can select only 5 AUDIOs");
//    } else {
//       alert();
//    }
//});
