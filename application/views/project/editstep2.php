...
<form method="POST" action="<?=base_url();?>project/add_post_step2/<?=$data1->id;?>" class="register-form_1" id="register-form222" enctype="multipart/form-data">
        <div class="top-header">
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="top-close"><input type="button" value="CLOSE" class="close-btn" name="submit" id="close-btnprojectstep2"></div>
                </div>
            </div>   
        </div>	   
						...
						<div class="form-group row">
							...
							<div class="file-upload">
								  <div class="file-select">
									...
									<input type="file" name="project_header_visual" value="<?php if($data1->project_header_visual){ echo $data1->project_header_visual;}?>" id="file" onchange="ValidateSingleInput(this);"  class="file" >
...
CKEDITOR.replace( 'project_description', {
	toolbar: [
		...
		{ name: 'basicstyles', items: [ 'Bold', 'Italic' ] }
						...
						<div class="form-group row">
							<label for="colFormLabelSm" class="col-sm-4 col-form-label col-form-label-sm">REGISTRATION FORM STATUS</label>
							<div class="col-sm-5">
								<select name="registration_form_status" id="registration_form_status" class="registration_form_status">
									<option value="">SELECT A REGISTRATION FORM STATUS</option>
									
									<option value="1" <?php if($data1->registration_form_status=='1'){ echo "selected"; }?>>DEFAULT</option>
									<option value="2" <?php if($data1->registration_form_status=='2'){ echo "selected"; }?>>OPEN REG</option>
									<option value="3" <?php if($data1->registration_form_status=='3'){ echo "selected"; }?>>CLOSE REG</option> 
                ...
                <div class="col-md-3 right-text">
                    <div class="tab right-tabs">
                       ...
                       <h6 class="image_err_fileformat1" style="color:red!important;font-weight: 600; "></h6>
							...
							<a href="#" class="action-btn backbtn" name="back" id="btn222212">BACK</a>
								<!--input type="submit" value="Back" class="action-btn backbtn" name="back" id="btn5"-->
		<input type="submit" value="Next" class="action-btn FloorSubmit" name="submit" id="btn222222">
                            </div>
                        </div>
                    </div>
                </div>
...
$(document).ready(function(){
 ...
 $.validator.setDefaults({
	...
	var registration_form_status=$('#registration_form_status').val();
...
var file_data = $('#file').prop('files')[0]; 
 ...
 formdata.append('registration_form_status', registration_form_status);
...
$.ajax({  
		...
		{ 
			...
			if (data) {
                           ...
                           $.fancybox.open({
                            src: '<?php echo base_url();?>project/editstep3/'+data ,
                            type: 'ajax',
                            ajax: { 
	...
	$('body').on('click', '#close-btnprojectstep2', function () {
            $.fancybox.close();
            
            window.location.href = "<?php echo base_url(); ?>project/deleteJunkRecord/<?php echo $data1->id; ?>/<?php echo $data1->group_id; ?>/<?php echo $data1->id; ?>/EDIT/2";
...
$("body").on('click','.backbtn',function(){
           ...
           $.fancybox.getInstance('close');
			...
			$.fancybox.open({
						...
						autoSize  : true,
						type        : "ajax",
						src         : "<?=base_url();?>project/editstep1/<?=$data1->id?>",
						ajax: { 
